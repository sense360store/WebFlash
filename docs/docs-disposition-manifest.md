...
| P2.4 | Cross-repo relink per the Phase 2 handoff map above. Retargeted (stubbed upstream paths → upstream `docs/archive-index.md`): `docs/sense360-webflash-status.md` (three links: `docs/first-release-gates.md`, `docs/first-release-dryrun-checklist.md`, `docs/sense360-room-bundles.md`) and `README.md` (two bare-path mentions of the first two). Verified resolving at upstream main, left as-is (KEEP surfaces): `docs/system-architecture.md` (incl. the deep anchor in `docs/architecture.md`), `docs/webflash-contract.md`, `docs/sense360-roadmap-status.md`, `docs/standing-invariants.md`, `config/product-catalog.json`, `config/preview-release-targets.json` successor mentions. Report-only residuals (guardrails 3 and 8, not rewritten): upstream-path links inside `docs/release-gates/WEBFLASH-FIRST-RELEASE-GATES-SYNC-001.md` and `WEBFLASH-FIRST-RELEASE-DRYRUN-HANDOFF-001.md` (still resolve via the upstream stubs; the same two docs also carry local `../../UPCOMING_PR.md` links now dangling since P2.3), `docs/security/SECURITY-AUDIT-2026-06.md` (one prose `UPCOMING_PR.md` mention), and `__tests__/fixtures/esphome-product-catalog.json` (vendored verbatim upstream data naming archived upstream docs) | #574 | 2026-07-05 | EXECUTED |
| P2.5 | Final link sweep and Phase 2 closure. Every kept doc's internal relative links verified resolving on disk (122 links across all root `*.md`, `docs/**`, and `firmware-signing/README.md`); all 15 unique cross-repo `sense360store/esphome-public` links verified returning 200 at main (the check environment's egress proxy 403-blocks `github.com` HTML pages, so release-tag links were verified via `raw.githubusercontent.com` at the tag ref and PR links via the `patch-diff.githubusercontent.com` endpoint). In-scope stale-link fixes: `docs/architecture.md` (unsupported-browser banner is surfaced by `scripts/install.js`, replacing the deleted `scripts/init-review.js` reference; the deleted `scripts/utils/esp-web-tools-overrides.js` overrides-surface sentence replaced with the current `scripts/install.js` rendering/lifecycle description), `README.md` (app build/version source of truth is the `webflash-app-version` meta tag in `index.html` read by `scripts/services/diagnostics.js`, replacing the deleted `scripts/build-info.js`), `CHANGELOG.md` (7 links) and `docs/sense360-webflash-status.md` (6 links) de-linked to inline code where historical entries referenced code files since deleted, text unchanged. Guardrail-8 report-only residuals, unchanged from the P2.4 record: 10 dangling relative links inside `docs/release-gates/WEBFLASH-FIRST-RELEASE-GATES-SYNC-001.md` and `docs/release-gates/WEBFLASH-FIRST-RELEASE-DRYRUN-HANDOFF-001.md` pointing at archived docs, the removed `UPCOMING_PR.md`, and the removed `scripts/data/kit-presets.js` (archived content recoverable via `docs/archive-index.md`) | #575 | 2026-07-05 | EXECUTED |

### Phase 2 closure

Phase 2 is COMPLETE for `sense360store/WebFlash`. PR list: P2.1 #571, P2.2
#572, P2.3 #573, P2.4 #574, P2.5 #575. Phase 3 (joint read-only verification
across `sense360store/esphome-public` and `sense360store/WebFlash`) is owed
and is performed outside this loop.
