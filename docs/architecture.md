...
## Authority model and the SOT commercial mirror (WEBFLASH-TAXONOMY-RECONCILE-001)

Three repositories own three different layers of truth, and WebFlash never
collapses them:

- **`sense360store/SOT`** owns commercial truth: bundle names, physical bundle
  contents, commercial lifecycle, visibility, buyability, and public
  commercial messaging.
- **`sense360store/esphome-public`** owns technical truth: physical board
  identity, firmware capability and lifecycle, config-string compatibility,
  and release/build declarations.
- **WebFlash** owns distribution: manifests, artifact names, signatures,
  checksums, install gates, release-channel execution, and the local
  installer presentation. Shopify is a presentation/transaction surface, not
  authority.

A local WebFlash fixture or mirror is never the authority it mirrors. WebFlash
keeps one checked-in snapshot of the SOT commercial surface —
[`scripts/data/sot-commercial-mirror.json`](../scripts/data/sot-commercial-mirror.json),
regenerated by `scripts/refresh-sot-mirror.py --sot-path <local SOT checkout>`
with the SOT source SHA recorded as provenance — so tests can guard, offline
and without network access, that customer copy never claims commercial
availability SOT has not declared. The mirror is synchronized evidence only:
it is never hand-authored, it feeds no install gate, and the customer-visible
consequence is purely negative (it forbids "buy now"/"on sale"-style copy
while SOT lists no bundle as available). The Step 1 customer surface renders
**room / firmware presets** from `scripts/data/kits.json` (see
[`docs/kit-configuration.md`](kit-configuration.md)): a preset resolves to a
real manifest config and never implies a buyable commercial bundle.
