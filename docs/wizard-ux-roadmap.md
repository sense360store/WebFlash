...
| --- | --- | --- | --- |
| **WF-UX-QUICK-001** | Remove the Admin note ([`index.html:1005`](../index.html#L1005)). Normalise browser-support copy across [`index.html:204`](../index.html#L204), [`:729`](../index.html#L729), [`:1012`](../index.html#L1012), [`scripts/init-review.js:19-21`](../scripts/init-review.js#L19), [`scripts/state.js:5075`](../scripts/state.js#L5075), [`scripts/layout/rescue-modal.js:21-23`](../scripts/layout/rescue-modal.js#L21), [`scripts/layout/sensor-health-panel.js:82`](../scripts/layout/sensor-health-panel.js#L82), [`scripts/layout/init-splitview.js:103-106`](../scripts/layout/init-splitview.js#L103), [`scripts/compat-config.js:617`](../scripts/compat-config.js#L617), and the four `scripts/capabilities.js` messages flagged in finding #3, to a single canonical phrase ("desktop Chrome, Edge, or Opera"). | Low | (a) No `Admin note` string in any rendered DOM. (b) `grep -n "Chrome or Edge"` in `index.html`/`scripts/` returns zero hits in user-visible markup. (c) `__tests__/preflight-capabilities.test.js`, `__tests__/release-channel-ui.test.js`, and `npm test -- --ci` pass without modification. (d) Release-One install behaviour and LED preview exposure unchanged. |
...
| **WF-HW-TEST-001** | Real-hardware flash + Improv handoff proof. Operator runs an actual flash against a Sense360 Core over Web Serial, captures the Improv handoff to Wi-Fi, exercises the post-flash panel, and records gaps that only show up on real serial. Findings filed as follow-up PRs. | Hardware / operator | (a) Transcript or screenshot pair attached. (b) Any new findings filed as separate PRs, not folded into this one. (c) Recovery / rescue flow exercised against a real device. (d) WF-LED-003 preview-channel exposure verified end-to-end (LED toggle → preview ack → install) at least once before larger UX restructure lands. |
