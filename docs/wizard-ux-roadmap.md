...
>
> **Amended by WF-UX-008.** The middle card's customer-facing label
> was reframed from *Custom configuration* to **Advanced setup**
> (with the guidance *"Use this only if your hardware doesn't match a
> kit"*). This is a pure copy change — the `data-start-path="custom"`
> data hook, the `configmode=custom` URL contract, the
> `[data-custom-path-panel]` panel id, and the back-compat
> `configmode=manual` alias are all unchanged.
...
> **Update — WF-UX-008.** Quick-win #3 is now an enforced guard, not a
> one-off check. WF-KIT-PRESETS-001 had re-introduced internal IDs into
> customer-visible Stage 1 copy (the planned-bundle `Tracked: RELEASE-…`
> meta lines, the `KIT-MATRIX-001` / `UPCOMING_PR.md` intro + source
> lines, and the `HW-005` TRIAC description), and the TRIAC
> module-availability `detail` still surfaced `HW-005 and COMPLIANCE-001`.
> WF-UX-008 replaced all of those with plain-language availability text +
> a next step, moved the internal references into developer/support-only
> data (`kit-presets.js` `upstreamRef` / `blockers`, the
> `module-availability.js` machine `reasonCode`), and added a standing
> `WF-UX-008 — no internal task/release/tracking IDs in customer-facing
> copy` guard in `__tests__/a11y-static-html.test.js` plus a
> customer-facing-field ID guard in `__tests__/kit-presets.test.js` so the
...
> **Status:** WF-UX-008 landed — customer-facing copy cleanup. Removed every
> internal task / release / tracking ID (`RELEASE-…-001`, `WF-IMPORT-…`,
> `HW-005`, `COMPLIANCE-001`, `KIT-MATRIX-001`, `UPCOMING_PR.md`,
> `import-firmware-sources.py`) from the customer-visible wizard surface —
> the Stage 1 planned-bundle card meta lines + planned intro + source line +
> TRIAC description in `index.html`, the `notAvailableReason` / `warning`
> copy in `scripts/data/kit-presets.js`, and the TRIAC advanced/manual-warning
> `detail` prose (per-variant override + config-string classifier) in
> `scripts/utils/module-availability.js` — and replaced them with
> plain-language availability text plus a next step ("Use the supported
> Sense360 Bathroom Kit — PoE for now, or check back later"). Internal
> references now live only in developer/support-only data (`kit-presets.js`
> `upstreamRef` / `blockers`, never rendered and with no consumers outside
> the data module) and the machine `module-availability.js` `reasonCode`
> (`hw-005-advanced-manual`, never rendered as prose). Also reframed the
> customer-facing **"Custom configuration"** path to **"Advanced setup"**
> (high-level goal #2) with customer-safe guidance ("Use this only if your
> hardware doesn't match a kit") across the Step 1 path card, panel header,
> kit-panel switch link, and the `kit-mode.js` announce — the
> `data-start-path="custom"` / `configmode=custom` data hooks and URL
> contract are unchanged. Guard tests added in
> `__tests__/a11y-static-html.test.js` (`WF-UX-008 — no internal
> task/release/tracking IDs in customer-facing copy`) and
> `__tests__/kit-presets.test.js`; the WF-TRIAC-001 `detail` assertions in
> `__tests__/module-availability.test.js` + `__tests__/wizard-state.test.js`
> were refreshed to assert the IDs are absent from the rendered prose and
> present only in `reasonCode`. Copy + test only — no firmware, manifest,
> source, kit, `REQUIRED_CONFIGS`, install-gate, preflight, freshness,
> release-channel, Release-One, LED-preview-exposure, or FanTRIAC-block
> change, and **no hardware behaviour was verified**.
...
| **WF-UX-008** ✅ landed | Customer-facing copy cleanup (resolves [Finding #10's class](#10-visible-internal-admin-note) for tracking IDs, and the WF-KIT-PRESETS-001 regression against [quick-win #3](#quick-wins)). Removed every internal task / release / tracking ID (`RELEASE-…-001`, `WF-IMPORT-…`, `HW-005`, `COMPLIANCE-001`, `KIT-MATRIX-001`, `UPCOMING_PR.md`, `import-firmware-sources.py`) from the customer-visible wizard surface: the Stage 1 planned-bundle `bundle-preset-card__meta` lines, `bundle-presets__planned-intro`, `bundle-presets__source`, and TRIAC `bundle-preset-card__description` in [`index.html`](../index.html); the `notAvailableReason` / `warning` copy in [`scripts/data/kit-presets.js`](../scripts/data/kit-presets.js); and the TRIAC advanced/manual-warning `detail` prose (the `ADVANCED_MANUAL_WARNING_DETAIL` per-variant override **and** the `classifyConfigString` config-string branch) in [`scripts/utils/module-availability.js`](../scripts/utils/module-availability.js). Replaced with plain-language availability text + a next step. Internal references now live only in developer/support-only data — `kit-presets.js` `upstreamRef` / `blockers` (never rendered; no consumers outside the data module) and the machine-readable `module-availability.js` `reasonCode` (`hw-005-advanced-manual`; never rendered as prose). Reframed the customer-facing **"Custom configuration"** path to **"Advanced setup"** (high-level goal #2) with customer-safe guidance ("Use this only if your hardware doesn't match a kit") across the Step 1 path card, panel header, kit-panel switch link, and the [`scripts/kit-mode.js`](../scripts/kit-mode.js) announce string. **Decisions and non-goals:** the `data-start-path="custom"` / `configmode=custom` / `data-custom-path-panel` data hooks and the URL contract are unchanged (pure copy reframe of the WF-UX-006 label); availability *states*, `reasonCode` values, the TRIAC install-gate acknowledgement (WF-TRIAC-001), the preview-channel acknowledgement (WF-LED-003), and the FanTRIAC HW-005 import block are all untouched; no firmware / manifest / `firmware-*.json` / `firmware/sources.json` / `scripts/data/kits.json` / `scripts/data/module-requirements.js` field values / `scripts/utils/release-channels.js` / `scripts/utils/firmware-readiness.js` / `sw.js` / `_headers` / `.github/workflows/*` change; **no hardware behaviour verified** (WF-HW-TEST-002 operator-evidence gate stays pending). | Low | (a) ✅ No internal task/release/tracking ID renders in the Stage 1 bundle section, the Step 1 path selector, or the custom/advanced + kit panels — pinned by `__tests__/a11y-static-html.test.js` `WF-UX-008 — no internal task/release/tracking IDs in customer-facing copy`. (b) ✅ Every planned bundle card shows a plain-language `Not available yet …` reason and each `notAvailableReason` points at a next step — pinned by the same block + `__tests__/kit-presets.test.js` `WF-UX-008 — each planned preset reason points the customer at a next step`. (c) ✅ Customer-facing preset fields (`displayName`/`shortName`/`description`/`warning`/`notAvailableReason`) carry no internal IDs; the IDs remain in `upstreamRef`/`blockers` — pinned by `__tests__/kit-presets.test.js` `WF-UX-008 — customer-facing preset fields expose no internal …`. (d) ✅ TRIAC `detail` prose no longer contains `HW-005`/`COMPLIANCE-001` but the `reasonCode` still does — refreshed `__tests__/module-availability.test.js` + `__tests__/wizard-state.test.js` TRIAC assertions. (e) ✅ Custom path button reads **Advanced setup** + the when-to-use guidance, keeping the WF-WIZARD-AVAIL-001 unavailable-module caveat — pinned by `__tests__/a11y-static-html.test.js`. (f) ✅ `npm test -- --ci` green (65 suites); Release-One install path, LED preview exposure, and FanTRIAC block all unchanged. |
