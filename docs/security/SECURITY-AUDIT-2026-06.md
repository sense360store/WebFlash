...
- **Status (2026-06-10): downstream half REMEDIATED — WF-H1-IMPORT-GATE; upstream half [`sense360store/esphome-public#779`](https://github.com/sense360store/esphome-public/pull/779) (SEC-ESP-BUILD-GATES-001).** The cross-repo importer ([`scripts/import-firmware-sources.py`](../../scripts/import-firmware-sources.py)) now scans every newly downloaded `.bin` after checksum verification against the denylist in [`scripts/check-firmware-default-credentials.py`](../../scripts/check-firmware-default-credentials.py) (kept in lock-step with the upstream release gate) and refuses the import — naming the source and the credential class, staging nothing — when the bytes carry any known default: the placeholder API encryption key (as the base64 literal **and** its decoded 32-byte material, the form flash actually stores), the default OTA password, the default web password (the default web username flags only in combination with a default web password), the default and burned fallback-AP passwords, and the CI test-lane WiFi values. The intentionally-public setup-network pair is the only exclusion (setup-only, no device control). Checksum-valid but credential-dirty is rejected, so a defaulted upstream artifact cannot reach the installer even if the upstream gate were bypassed or the asset predates it. **Caveat — live binaries:** the nine imported stable/preview binaries currently published predate the upstream fix, still match this denylist, and stay live until the tracked rebuild + clean re-import lands (`UPCOMING_PR.md` → WF-H1-REIMPORT-CLEAN-001, blocked on #779's unprovisioned release artifacts); the in-tree Rescue build scans clean. Re-importing the defaulted binaries through the gate is now blocked; the gate applies to new imports and does not delete or alter what is already published. Per-device secret provisioning remains the upstream follow-up (`SEC-ESP-PROVISIONING-001`).
