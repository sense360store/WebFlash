...
## Firmware provenance and manifest validation

Flashing is supply-chain-sensitive: the wizard hands a binary to ESP Web Tools that overwrites the device's flash. WebFlash treats every manifest entry as untrusted until it passes a static provenance gate (`scripts/utils/firmware-provenance.js`) **before** the SHA-256/signature download begins.
...
|---|---|---|
| `sha256` | Cryptographic checksum verified at runtime against the downloaded binary | `gen-manifests.py` (digest of `.bin`) |
| `signature` | Salted SHA-256 signature blob, also re-verified at runtime | `gen-manifests.py` (digest + `Sense360 Firmware Signing Salt v1`) |
...
Step 5 renders a **Provenance verified** panel inside the firmware card listing the seven verification checks (SHA-256, signature, source commit, declared file size, changelog, file-size sanity, lifecycle status) with pass/fail icons. The source commit links out to the upstream commit URL when available. If anything fails, the panel turns red, the install button stays disabled, and the helper banner reproduces the blocking reason.

### Tooling for maintainers

- `python3 scripts/gen-manifests.py --strict-validate` promotes provenance findings (missing source commit, suspicious file size, etc.) from warnings to build failures.
- `WEBFLASH_SOURCE_COMMIT=<sha> python3 scripts/gen-manifests.py` overrides the source commit (useful when running outside a git checkout).
- `python3 scripts/gen-manifests.py --source-url-template "https://example.com/builds/{commit}"` customises the per-build `source_url`.
- `npm test -- firmware-provenance` runs both the unit and the install-gate integration tests for this feature.
